...
paf = []
with open(RDNA_V1_0_PAF, 'r') as f:
    for line in f:
...
rdna_offsets = {}
rdna_chrlen = {}
with pysam.AlignmentFile('rDNA_to_v1.1.sam') as f:
    for r in f:
...
for idx_p, p in enumerate(paf_final):
    ...
    for i_cg, op in enumerate(cg_ops):
        ...
        if op == 'M':
            ...
            print(f'{match}\t0\t{cg_lens[i_cg]}')
            match = None
        elif op == 'D':
            assert match is not None
            print(f'{match}\t{cg_lens[i_cg]}\t0')
            match = None
        else:
            print('error: unexpected cigar op', op)
            exit(1)
    print(f'{match}\n')
